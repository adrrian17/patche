...
- packages (`packages/*`);
- app (`apps/web`), including Cloudflare/D1 integration tests (`*.integration.ts`);
...
1. Run the smallest owner and sibling tests with `bun test <path-or-filter>`;
   for Workers/D1 integration tests run `bun run test:integration`.
2. For removed source greps or plan assertions, run the executable script or
   dry-run that owns the real contract.
3. Run `bun run check` on changed paths, then `git diff --check`.
4. Run `bun run check-types` and the full `bun test`.
